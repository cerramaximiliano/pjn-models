// Modelo para FMP - Justicia Federal de Mar del Plata
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFMPSchema = new Schema({
  ...
  fuero: { type: String, default: 'FMP', index: true },
  ...
  timestamps: true,
  collection: 'causas_fmp'
});

// Índices compuestos
CausasFMPSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFMPSchema.index({ juzgado: 1, secretaria: 1 });
CausasFMPSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFMPSchema.index({ 'processingLock.expiresAt': 1 });
CausasFMPSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFMPSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFMPSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFMPSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFMPSchema.add(require("../shared/sentencias-scan-fields"));
CausasFMPSchema.add(require("../shared/etapa-procesal-fields"));
