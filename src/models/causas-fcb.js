// Modelo para FCB - Justicia Federal de Córdoba
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFCBSchema = new Schema({
  ...
  fuero: { type: String, default: 'FCB', index: true },
  ...
  timestamps: true,
  collection: 'causas_fcb'
});

// Índices compuestos
CausasFCBSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFCBSchema.index({ juzgado: 1, secretaria: 1 });
CausasFCBSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFCBSchema.index({ 'processingLock.expiresAt': 1 });
CausasFCBSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFCBSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFCBSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFCBSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFCBSchema.add(require("../shared/sentencias-scan-fields"));
CausasFCBSchema.add(require("../shared/etapa-procesal-fields"));

module.exports = mongoose.models.CausasFCB || mongoose.model("CausasFCB", CausasFCBSchema);