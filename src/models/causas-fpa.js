// Modelo para FPA - Justicia Federal de Paraná
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFPASchema = new Schema({
  ...
  fuero: { type: String, default: 'FPA', index: true },
  ...
  timestamps: true,
  collection: 'causas_fpa'
});

// Índices compuestos
CausasFPASchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFPASchema.index({ juzgado: 1, secretaria: 1 });
CausasFPASchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFPASchema.index({ 'processingLock.expiresAt': 1 });
CausasFPASchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFPASchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFPASchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFPASchema.statics.safeSave = async function(docData) {
    try {
...
CausasFPASchema.add(require("../shared/sentencias-scan-fields"));
CausasFPASchema.add(require("../shared/etapa-procesal-fields"));

module.exports = mongoose.models.CausasFPA || mongoose.model("CausasFPA", CausasFPASchema);