...
// Índices compuestos
CausasCCFSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasCCFSchema.index({ juzgado: 1, secretaria: 1 });
CausasCCFSchema.index({ verified: 1, isError: 1 });
CausasCCFSchema.index({ 'processingLock.expiresAt': 1 });
CausasCCFSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasCCFSchema.index({
    source: 1,
    verified: 1,
    isValid: 1,
    update: 1,
...
CausasCCFSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasCCFSchema.statics.safeSave = async function(docData) {
    try {
...
CausasCCFSchema.add(require("../shared/etapa-procesal-fields"));
