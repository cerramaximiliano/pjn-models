// Modelo para FBB - Justicia Federal de Bahia Blanca
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFBBSchema = new Schema({
  ...
  fuero: { type: String, default: 'FBB', index: true },
  ...
  timestamps: true,
  collection: 'causas_fbb'
});

// Índices compuestos
CausasFBBSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFBBSchema.index({ juzgado: 1, secretaria: 1 });
CausasFBBSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFBBSchema.index({ 'processingLock.expiresAt': 1 });
CausasFBBSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFBBSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFBBSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFBBSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFBBSchema.add(require("../shared/etapa-procesal-fields"));
