// Modelo para FCT - Justicia Federal de Corrientes
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFCTSchema = new Schema({
  ...
  fuero: { type: String, default: 'FCT', index: true },
  ...
  timestamps: true,
  collection: 'causas_fct'
});

// Índices compuestos
CausasFCTSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFCTSchema.index({ juzgado: 1, secretaria: 1 });
CausasFCTSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFCTSchema.index({ 'processingLock.expiresAt': 1 });
CausasFCTSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFCTSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFCTSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFCTSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFCTSchema.add(require("../shared/sentencias-scan-fields"));
CausasFCTSchema.add(require("../shared/etapa-procesal-fields"));
