// Modelo para FCR - Justicia Federal de Comodoro Rivadavia
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFCRSchema = new Schema({
  ...
  fuero: { type: String, default: 'FCR', index: true },
  ...
  timestamps: true,
  collection: 'causas_fcr'
});

// Índices compuestos
CausasFCRSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFCRSchema.index({ juzgado: 1, secretaria: 1 });
CausasFCRSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFCRSchema.index({ 'processingLock.expiresAt': 1 });
CausasFCRSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFCRSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFCRSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFCRSchema.statics.safeSave = async function(docData) {
    try {
...
module.exports = mongoose.model('CausasFCR', CausasFCRSchema);
