...
CausasCPESchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasCPESchema.index({
    source: 1,
    verified: 1,
    isValid: 1,
    update: 1,
...
CausasCPESchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasCPESchema.statics.safeSave = async function(docData) {
    try {
...
CausasCPESchema.add(require("../shared/etapa-procesal-fields"));
