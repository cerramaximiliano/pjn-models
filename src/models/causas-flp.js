// Modelo para FLP - Justicia Federal de La Plata
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFLPSchema = new Schema({
  ...
  timestamps: true,
  collection: 'causas_flp'
});

// Índices compuestos
CausasFLPSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFLPSchema.index({ juzgado: 1, secretaria: 1 });
CausasFLPSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFLPSchema.index({ 'processingLock.expiresAt': 1 });
CausasFLPSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFLPSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFLPSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFLPSchema.statics.safeSave = async function(docData) {
    try {
