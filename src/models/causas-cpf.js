// Modelo para CPF - Camara Federal de Casación Penal
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasCPFSchema = new Schema({
  ...
  fuero: { type: String, default: 'CPF', index: true },
  ...
  timestamps: true,
  collection: 'causas_cpf'
});

// Índices compuestos
CausasCPFSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasCPFSchema.index({ juzgado: 1, secretaria: 1 });
CausasCPFSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasCPFSchema.index({ 'processingLock.expiresAt': 1 });
CausasCPFSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasCPFSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasCPFSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasCPFSchema.statics.safeSave = async function(docData) {
    try {
...
CausasCPFSchema.add(require("../shared/etapa-procesal-fields"));
