// Modelo para FPO - Justicia Federal de Posadas
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFPOSchema = new Schema({
  ...
  fuero: { type: String, default: 'FPO', index: true },
  ...
  timestamps: true,
  collection: 'causas_fpo'
});

// Índices compuestos
CausasFPOSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFPOSchema.index({ juzgado: 1, secretaria: 1 });
CausasFPOSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFPOSchema.index({ 'processingLock.expiresAt': 1 });
CausasFPOSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFPOSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFPOSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFPOSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFPOSchema.add(require("../shared/sentencias-scan-fields"));
CausasFPOSchema.add(require("../shared/etapa-procesal-fields"));
