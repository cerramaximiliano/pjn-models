// Modelo para FSA - Justicia Federal de Salta
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFSASchema = new Schema({
  ...
  fuero: { type: String, default: 'FSA', index: true },
  ...
  timestamps: true,
  collection: 'causas_fsa'
});

// Índices compuestos
CausasFSASchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFSASchema.index({ juzgado: 1, secretaria: 1 });
CausasFSASchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFSASchema.index({ 'processingLock.expiresAt': 1 });
CausasFSASchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFSASchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFSASchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFSASchema.statics.safeSave = async function(docData) {
    try {
...
CausasFSASchema.add(require("../shared/sentencias-scan-fields"));
CausasFSASchema.add(require("../shared/etapa-procesal-fields"));

module.exports = mongoose.models.CausasFSA || mongoose.model("CausasFSA", CausasFSASchema);