// Modelo para FSM - Fuero de Seguridad Social Ministerial
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFSMSchema = new Schema({
  ...
  fuero: { type: String, default: 'FSM', index: true },
  ...
  timestamps: true,
  collection: 'causas_fsm'
});

// Índices compuestos
CausasFSMSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFSMSchema.index({ juzgado: 1, secretaria: 1 });
CausasFSMSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFSMSchema.index({ 'processingLock.expiresAt': 1 });
CausasFSMSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFSMSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFSMSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFSMSchema.statics.safeSave = async function(docData) {
    try {
