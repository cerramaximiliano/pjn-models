// Modelo para CPN - Camara Nacional Casacion Penal
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasCPNSchema = new Schema({
  ...
  fuero: { type: String, default: 'CPN', index: true },
  ...
  timestamps: true,
  collection: 'causas_cpn'
});

// Índices compuestos
CausasCPNSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasCPNSchema.index({ juzgado: 1, secretaria: 1 });
CausasCPNSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasCPNSchema.index({ 'processingLock.expiresAt': 1 });
CausasCPNSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasCPNSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasCPNSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasCPNSchema.statics.safeSave = async function(docData) {
    try {
