// Modelo para FRE - Justicia Federal de Resistencia
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFRESchema = new Schema({
  ...
  fuero: { type: String, default: 'FRE', index: true },
  ...
  timestamps: true,
  collection: 'causas_fre'
});

// Índices compuestos
CausasFRESchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFRESchema.index({ juzgado: 1, secretaria: 1 });
CausasFRESchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFRESchema.index({ 'processingLock.expiresAt': 1 });
CausasFRESchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFRESchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFRESchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFRESchema.statics.safeSave = async function(docData) {
    try {
...
CausasFRESchema.add(require("../shared/sentencias-scan-fields"));
CausasFRESchema.add(require("../shared/etapa-procesal-fields"));
