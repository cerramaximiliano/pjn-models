// Modelo para FTU - Justicia Federal de Tucuman
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFTUSchema = new Schema({
  ...
  fuero: { type: String, default: 'FTU', index: true },
  ...
  timestamps: true,
  collection: 'causas_ftu'
});

// Índices compuestos
CausasFTUSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFTUSchema.index({ juzgado: 1, secretaria: 1 });
CausasFTUSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFTUSchema.index({ 'processingLock.expiresAt': 1 });
CausasFTUSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFTUSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFTUSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFTUSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFTUSchema.add(require("../shared/etapa-procesal-fields"));
