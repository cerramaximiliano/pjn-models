// Modelo para FGR - Justicia Federal de General Roca
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFGRSchema = new Schema({
  ...
  fuero: { type: String, default: 'FGR', index: true },
  ...
  timestamps: true,
  collection: 'causas_fgr'
});

// Índices compuestos
CausasFGRSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFGRSchema.index({ juzgado: 1, secretaria: 1 });
CausasFGRSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFGRSchema.index({ 'processingLock.expiresAt': 1 });
CausasFGRSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFGRSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFGRSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFGRSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFGRSchema.add(require("../shared/sentencias-scan-fields"));
CausasFGRSchema.add(require("../shared/etapa-procesal-fields"));
