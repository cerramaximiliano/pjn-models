// Modelo para FRO - Justicia Federal de Rosario
const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const CausasFROSchema = new Schema({
  ...
  fuero: { type: String, default: 'FRO', index: true },
  ...
  timestamps: true,
  collection: 'causas_fro'
});

// Índices compuestos
CausasFROSchema.index({ number: 1, year: 1, incidente: 1, fuero: 1 }, { unique: true });
CausasFROSchema.index({ juzgado: 1, secretaria: 1 });
CausasFROSchema.index({ verified: 1, isError: 1 });

// Índices para processingLock
CausasFROSchema.index({ 'processingLock.expiresAt': 1 });
CausasFROSchema.index({ 'processingLock.workerId': 1 });

// Índice compuesto para la query principal del app-update-worker
CausasFROSchema.index({
  source: 1,
  verified: 1,
  isValid: 1,
  update: 1,
...
CausasFROSchema.index({ 'scrapingProgress.skipUntil': 1 });

// Método estático para manejar errores E11000 (duplicados)
CausasFROSchema.statics.safeSave = async function(docData) {
    try {
...
CausasFROSchema.add(require("../shared/sentencias-scan-fields"));
CausasFROSchema.add(require("../shared/etapa-procesal-fields"));
